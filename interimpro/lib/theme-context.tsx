...
export function applyTheme(accent: string, dark: boolean) {
  ...
  if (dark) {
    ...
    r.style.setProperty('--bg-card', '#1e2433')
    r.style.setProperty('--bg-input', '#252d3d')
    r.style.setProperty('--bg-hover', '#2a3447')
    r.style.setProperty('--bg-modal', '#1a2235')
    r.style.setProperty('--text', '#f1f5f9')
    r.style.setProperty('--text-muted', '#94a3b8')
    r.style.setProperty('--text-dim', '#64748b')
    r.style.setProperty('--border', '#2a3447')
    r.style.setProperty('--sidebar-bg', '#0d1526')
    r.style.setProperty('--topbar-border', '#1e293b')
    r.style.setProperty('--shadow', 'rgba(0,0,0,0.6)')
    r.style.setProperty('--overlay', 'rgba(0,0,0,0.7)')
    r.setAttribute('data-theme', 'dark')
  } else {
    r.style.setProperty('--bg', '#f1f5f9')
    r.style.setProperty('--bg-card', '#ffffff')
    r.style.setProperty('--bg-input', '#f8fafc')
    r.style.setProperty('--bg-hover', '#e2e8f0')
    r.style.setProperty('--bg-modal', '#ffffff')
    ...
    r.style.setProperty('--topbar-border', '#e2e8f0')
    r.style.setProperty('--shadow', 'rgba(0,0,0,0.1)')
    r.style.setProperty('--overlay', 'rgba(0,0,0,0.4)')
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  // Appliquer le thème depuis le cache local IMMÉDIATEMENT (pas de flash)
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_e, session) => {
      ...
      // Charger les prefs en arrière-plan
      ...
      if (data) {
        ...
        const o = Number(data.objectif_heures_mensuel) || 152
