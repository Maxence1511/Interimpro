...
type Etab = { id: string; nom: string; groupe?: string; type?: string; taux_horaire: number; telephone?: string; email?: string; notes?: string; archived: boolean }
type Mission = { etablissement_id: string; heures?: number; salaire_estime?: number }
type Form = { nom: string; groupe: string; type: string; taux_horaire: number; telephone: string; email: string; notes: string }
...
export default function EtablissementsPage() {
  ...
  const empty: Form = { nom: '', groupe: '', type: 'Hopital', taux_horaire: 14, telephone: '', email: '', notes: '' }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editing) {
      await supabase.from('etablissements').update(data).eq('id', editing.id)
    } else {
      await supabase.from('etablissements').insert(data)
    }
    setShowForm(false); setEditing(null); setForm(empty); load()
  }

  const archive = async (etab: Etab) => {
  ...
  const stats = (id: string) => {
    const ms = missions.filter(m => m.etablissement_id === id)
    return { count: ms.length, heures: ms.reduce((a, x) => a + (x.heures || 0), 0), revenus: ms.reduce((a, x) => a + (x.salaire_estime || 0), 0) }
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '2px' }}>Établissements</h1>
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{etabs.filter(e => !e.archived).length} actifs · {etabs.filter(e => e.archived).length} archivés</p>
        </div>
      ...
      : displayed.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-secondary)' }}>
          ...
          {!archives && <button onClick={() => setShowForm(true)} style={{ color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px' }}>+ Ajouter un établissement</button>}
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '14px' }}>
          {displayed.map(etab => {
            const s = stats(etab.id)
            const color = TYPE_COLORS[etab.type || 'Autre'] || '#94a3b8'
            return (
              <div key={etab.id} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px', opacity: etab.archived ? 0.65 : 1 }}>
                ...
                <div style={{ marginBottom: '14px', fontSize: '13px', color: 'var(--text-secondary)' }}>💰 {etab.taux_horaire}€/h</div>
                <div style={{ display: 'flex', gap: '8px' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={e => { if (e.target === e.currentTarget) setShowForm(false) }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '460px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px' }}>
              <h2 style={{ fontSize: '17px', fontWeight: 700, color: 'var(--text-primary)' }}>{editing ? 'Modifier' : 'Nouvel établissement'}</h2>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
              ...
              <div><label style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: 'var(--text-secondary)', marginBottom: '4px' }}>Groupe / Réseau</label><input value={form.groupe} onChange={e => setForm({ ...form, groupe: e.target.value })} placeholder="Korian..." style={inp} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                ...
                <div><label style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: 'var(--text-secondary)', marginBottom: '4px' }}>Taux horaire (€)*</label><input required type="number" step="0.5" value={form.taux_horaire} onChange={e => setForm({ ...form, taux_horaire: Number(e.target.value) })} style={inp} /></div>
              </div>
              <div><label style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: 'var(--text-secondary)', marginBottom: '4px' }}>Téléphone</label><input value={form.telephone} onChange={e => setForm({ ...form, telephone: e.target.value })} placeholder="04 XX XX XX XX" style={inp} /></div>
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '4px' }}>
                ...
                <button type="submit" style={{ padding: '10px 18px', borderRadius: '8px', background: 'var(--accent)', color: 'white', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 600 }}>{editing ? 'Enregistrer' : 'Créer'}</button>
              </div>
            </form>
          </div>
        </div>
