...
export default function ExchangePage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.exchangeCodeForSession(code)
      .then(({ data, error }) => {
        ...
        if (data.session) {
          setStatus('Connecte !')
          // Petite pause pour que localStorage soit bien ecrit
          setTimeout(() => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fdf2f8 0%, #fce7f3 30%, #f5d0fe 60%, #ede9fe 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
