    ...
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#fdf2f8 0%,#fce7f3 40%,#ede9fe 100%)', display:'flex', alignItems:'center', justifyContent:'center', padding:20, position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ width:'100%', maxWidth:400, position:'relative', zIndex:1 }}>
        ...
        <div style={{ background:'rgba(255,255,255,0.92)', backdropFilter:'blur(20px)', border:'1px solid rgba(232,123,249,0.15)', borderRadius:20, padding:28, boxShadow:'0 8px 40px rgba(147,51,234,0.08)' }}>
          <h2 style={{ fontSize:17, fontWeight:700, color:'#3b0764', marginBottom:20, textAlign:'center' }}>
            {mode === 'login' ? 'Connexion' : 'Inscription'}
          </h2>

          {error && <div style={{ padding:'9px 13px', borderRadius:9, background:'rgba(239,68,68,.08)', border:'1px solid rgba(239,68,68,.2)', fontSize:13, color:'#dc2626', marginBottom:14 }}>⚠️ {error}</div>}
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <div>
              <label style={{ display:'block', fontSize:12, fontWeight:600, color:'#7c3aed', marginBottom:5 }}>Email</label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="vous@exemple.com" required style={inp}/>
            </div>
            <div>
              <label style={{ display:'block', fontSize:12, fontWeight:600, color:'#7c3aed', marginBottom:5 }}>Mot de passe</label>
          ...
          <button onClick={()=>setMode(mode==='login'?'signup':'login')} style={{ width:'100%', marginTop:14, fontSize:13, color:'#9333ea', background:'none', border:'none', cursor:'pointer', fontWeight:500 }}>
